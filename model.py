...
# File: model.py
# Description: Model for the application
# Date: 23/04/2022
...
class QAModel(nn.Module):
    ...
    def forward(self, input_enc: dict):
        ...
        # Passing through the linear layer, to learn it's parameters
        ...
        # out_cls_logits = F.max_pool1d(out_cls, kernel_size=out_cls.shape[-1]).squeeze(-1).permute(1,0)  # (batch_size, 2)
        ...
        if start_pos is not None and end_pos is not None:
            ...
            # is_impossible = torch.tensor(is_impossible, dtype=torch.long).detach().reshape(-1, 1).squeeze(-1)  # (batch_size)
